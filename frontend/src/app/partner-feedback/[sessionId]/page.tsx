			...
			try {
				...
				setTimeout(() => {
					setFeedback({
						...
						aiGoodPoints:
							"会話中の笑顔が自然で、相手に好印象を与えていました。視線も適切に相手を見ており、傾聴の姿勢が伝わってきました。質問も具体的で、会話を深める工夫が見られました。",
						aiImprovementPoints:
							"時折、相手の話を遮ってしまう場面がありました。もう少し間を取って、相手の話を最後まで聞く意識を持つとより良いでしょう。また、声のトーンが少し単調になる場面があったので、感情を込めて話すとさらに魅力的になります。",
						aiOverallScore: 78,
						gestureMetrics: {
						...
						partnerRating: 4,
						partnerComment:
							"とても誠実に話を聞いてくださって嬉しかったです！もう少しリラックスして話すと、さらに自然な会話になると思います。",
										...
										<div className="flex items-center gap-3">
											<div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
												<div
													...
													style={{
														width: `${feedback.gestureMetrics.smileIntensityAvg * 100}%`,
													}}
												/>
											</div>
											<span className="text-sm font-semibold">
												{(
													feedback.gestureMetrics.smileIntensityAvg * 100
												).toFixed(0)}
												%
											</span>
										</div>
										...
										<div className="flex items-center gap-3">
											<div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
												<div
													className="h-full bg-primary transition-all"
													style={{
														width: `${feedback.gestureMetrics.gazeScoreAvg * 100}%`,
													}}
												/>
											</div>
											<span className="text-sm font-semibold">
												{(feedback.gestureMetrics.gazeScoreAvg * 100).toFixed(
													0,
												)}
												%
											</span>
