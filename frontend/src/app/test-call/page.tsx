		...
		<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background to-muted">
			<Card className="p-8 max-w-md w-full space-y-6">
				<div className="text-center space-y-2">
					...
					<h1 className="text-2xl font-bold text-foreground">WebRTC通話テスト</h1>
					<p className="text-muted-foreground">開発者向けのテストページです</p>
				</div>

				<div className="space-y-4">
					<div className="space-y-2">
						<label className="text-sm font-medium text-foreground">セッションID</label>
						<input
							type="text"
							value={sessionId}
						...
						/>
						<p className="text-xs text-muted-foreground">両方のブラウザで同じセッションIDを使用してください</p>
					</div>
					...
					<div className="space-y-3 pt-4">
						...
						<div className="space-y-2 text-sm text-muted-foreground">
							...
							<ol className="list-decimal list-inside space-y-1 ml-2">
								...
								<li>両ウィンドウで接続処理完了後、自動的にカウントダウン開始</li>
							</ol>
						</div>
					</div>
					...
					<div className="flex flex-col gap-2 pt-2">
						<Button onClick={joinAsUser} size="lg" className="w-full">
							<Video className="w-5 h-5 mr-2" />ユーザーとして参加
						</Button>
						<Button onClick={joinAsPartner} variant="outline" size="lg" className="w-full">
							<Video className="w-5 h-5 mr-2" />パートナーとして参加（新しいウィンドウ）
						</Button>
					</div>
					...
					<Card className="p-4 border border-primary/30 bg-primary/5 space-y-3">
						...
						<div className="grid grid-cols-2 gap-2 text-xs">
							<div className={`rounded-md px-2 py-2 border ${userConnected ? 'border-green-500 bg-green-500/10 text-green-700' : 'border-border text-muted-foreground'}`}>ユーザー: {userConnected ? '接続' : '未接続'}</div>
							<div className={`rounded-md px-2 py-2 border ${partnerConnected ? 'border-green-500 bg-green-500/10 text-green-700' : 'border-border text-muted-foreground'}`}>パートナー: {partnerConnected ? '接続' : '未接続'}</div>
						</div>
						{!timerStarted && (
							<p className="text-xs text-muted-foreground">両方が接続すると5分タイマーが開始します。</p>
						)}
						{timerStarted && secondsRemaining !== null && (
							<div className="text-center">
								<p className="text-sm font-medium">残り時間: <span className={`font-bold ${secondsRemaining <= 30 ? 'text-red-600 animate-pulse' : 'text-foreground'}`}>{formatTime(secondsRemaining)}</span></p>
								<Button onClick={endCall} size="sm" variant="outline" className="mt-2">会話を終了する</Button>
							</div>
						)}
					</Card>
					...
					<div className="pt-2">
						<p className="text-xs text-muted-foreground"><strong>注意:</strong> バックエンド（port 8787）が起動していることを確認してください。</p>
					</div>
				</div>
			</Card>
		</div>
