...
import { Heart, Calendar, Clock, User, CheckCircle, XCircle, AlertCircle } from "lucide-react";
      ...
      <main className="flex-1 p-4 sm:p-6">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-foreground">予定中のセッション</h2>
            {mockSessions.filter((s) => s.status === "scheduled" || s.status === "active").length === 0 ? (
          ...
          {/* Past Sessions */}
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-foreground">過去のセッション</h2>
            {mockSessions.filter((s) => s.status === "completed" || s.status === "cancelled").length === 0 ? (
              <Card className="p-6 border-2">
                <p className="text-center text-muted-foreground">
                  過去のセッションはまだありません
                </p>
