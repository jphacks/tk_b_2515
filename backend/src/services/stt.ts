...
export async function getVoiceById(apiKey: string, voiceId: string): Promise<Voice | null> {
  const voices = await getVoices(apiKey);
  return voices.find(v => v.voiceId === voiceId) || null;
}

export async function speechToText(apiKey: string, audioFile: File): Promise<string> {
  const client = new ElevenLabsClient({
    apiKey,
  });
