			...
			content: {
				"application/json": {
					schema: z.object({
						...
						relationshipStage: z
							.enum(["shy", "friendly", "open"])
							.optional()
							.openapi({
								description: "親密度レベル（省略時はshy）",
							}),
						backgroundKey: z
							.enum(["library", "classroom", "xmas"])
							.optional()
							.openapi({
...
conversation.openapi(generateResponseRoute, async (c) => {
	try {
		...
		const {
			...
			backgroundKey,
			adviceCompletedIds,
		} = c.req.valid("json") as GenerateReq;
		...
		try {
			...
			const personasModule = (await import(
				"../../config/avatar-personas.json",
				{
					assert: { type: "json" },
				}
...
conversation.openapi(generateFeedbackRoute, async (c) => {
	try {
		...
		const { sessionId, backgroundKey, adviceCompletedIds } =
			c.req.valid("json");
		...
		if (existingFeedback) {
			savedFeedback = await prisma.feedback.update({
				...
				data: {
					...
					voiceMetrics:
						feedbackData.voiceMetrics as unknown as Prisma.InputJsonValue,
				},
			});
